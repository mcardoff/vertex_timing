...
//
// Output PDF: ../parameter_sweep/maxtrackpt_sweep.pdf
//
// ── Configuration ────────────────────────────────────────────────────────────
...
static constexpr double   PT_MAX        = 40.0; // GeV
...
int main() {
  ...
  for (int r = 0; r < N_REGIONS; ++r) {
    for (int ic = 0; ic < nCuts; ++ic) {
      resHists[r][ic] = new TH1D(
        ...
        Form("TRKPTZ #Deltat (%s, maxTrkPt=%.1f GeV);#Delta t [ps];Entries",
  ...
  // --- Plotting ---
  TCanvas* canvas = new TCanvas("canvas", "Max Track pT Sweep Results", 800, 600);
  canvas->SetLeftMargin(0.15);

  const std::string outFile = std::string(OUT_DIR) + "/maxtrackpt_sweep.pdf";
  canvas->Print((outFile + "[").c_str());

  const double xLo = PT_MIN - PT_STEP * 0.5;
  const double xHi = PT_MAX + PT_STEP * 0.5;
  const char*  xtitle = "Max Track p_{T} [GeV]";
  ...
  saveGraphPair(outFile, canvas, grEff[0], grEff[1],
    ...
    xLo, xHi, 0.0, 1.15, HS_TRACK_SPLIT,
    0.99, "99% Efficiency");
  ...
  // Plot 4: Mean purity of selected cluster vs maxTrackPt
  saveGraphPair(outFile, canvas, grPurity[0], grPurity[1],
    "Mean Selected-Cluster Purity vs. Max Track p_{T}",
    xtitle, "Mean Purity (HS p_{T} fraction)",
    xLo, xHi, 0.0, 1.05, HS_TRACK_SPLIT,
    0.75, "75% purity");

  // Plot 5: Mean track count of selected cluster vs maxTrackPt
  double tcYMax = 1.0;
  for (int r = 0; r < N_REGIONS; ++r) {
    for (int ic = 0; ic < nPlot[r]; ++ic) {
      double xv, yv;
  ...
  saveGraphPair(outFile, canvas, grTrackCount[0], grTrackCount[1],
    "Mean Selected-Cluster Track Count vs. Max Track p_{T}",
  ...
  for (int r = 0; r < N_REGIONS; ++r) {
    ...
    std::cout << Form("%-12s  %-10s  %-10s  %-14s  %-10s  %-12s\n",
                      "maxTrackPt", "Efficiency", "CoreSigma", "MeanNClusters", "MeanPurity", "MeanNTracks");
    for (int ic = 0; ic < nPlot[r]; ++ic) {
      ...
      (void)dummy;
      std::cout << Form("%-12.1f  %-10.4f  %-10.2f  %-14.2f  %-10.4f  %-12.2f\n",
