...
int main() {
  ...
  while (reader.Next()) {
    ...
    // ── Reference track pT histogram at the loosest min-pT cut ──────────────
    {
      std::vector<int> refTracks = getAssociatedTracks(&branch, cuts[0], MAX_TRACK_PT, 3.0);
  ...
  for (int r = 0; r < N_REGIONS; ++r) {
    ...
    for (int ic = 0; ic < nPlot[r]; ++ic) {
      ...
      h->SetTitle(Form("TRKPTZ #Deltat shape (%s);#Delta t [ps];Normalised Entries",
                       regionLabel[r].c_str()));
      resLabels6.push_back(Form("%.1f GeV", cuts[ic]));
  ...
  // Plot 7: Track pT distribution at loosest min-pT cut, with sweep cut markers (one page)
  {
    ...
    h0->GetXaxis()->SetRangeUser(0.0, 10.0);
    h0->SetTitle("Track p_{T} Distribution (min p_{T} sweep);Track p_{T} [GeV];Normalised Entries");
    ...
    LegendBox lbPt = bestLegendCornerHist({h0, h1}, 0.0, 10.0, yMinPt, yMaxPt, 3, true);
    ...
    legPt->AddEntry(dummyVL, "Sweep cuts (every 0.5 GeV)", "l");
    legPt->Draw("SAME");
    canvas->Print(outFile.c_str());
    canvas->SetLogy(false);
  ...
  std::vector<std::string> minPtLabels(nCuts);
  for (int ic = 0; ic < nCuts; ++ic)
    minPtLabels[ic] = Form("%.1f GeV", cuts[ic]);
  ...
  saveRocPlot(outFile, canvas,
    effLow, purLow, effHigh, purHigh, minPtLabels,
    "Purity vs. Efficiency (min p_{T} sweep)",
    "Overall Efficiency", "Mean Cluster Purity",
    HS_TRACK_SPLIT);

  saveRocPlot(outFile, canvas,
    sigLow, purLow, sigHigh, purHigh, minPtLabels,
    "Purity vs. Core #sigma (min p_{T} sweep)",
    "Core #sigma [ps]", "Mean Cluster Purity",
    HS_TRACK_SPLIT);

  saveRocPlot(outFile, canvas,
    sigLow, effLow, sigHigh, effHigh, minPtLabels,
    "Efficiency vs. Core #sigma (min p_{T} sweep)",
