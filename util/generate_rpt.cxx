...
auto min_jetpt         =  40.0; // self explanatory
...
auto max_vtx_dz        =  0.1;  // max error for reco HS vertex z
auto max_nsigma_z      =  3.0;  // how close a track can be to PV
auto max_nsigma_t      =  3.0;  // 2σ_t window per TDR Section 3.3.2
auto t_cut = 60;
...
int main() {
  ...
  // Colors from clustering_constants.h (kP10/kP6 palette via MyUtl namespace)
...
// Add at the start of main()
  ...
  while (reader.Next()) {
    ...
    // Require reco HS vertex within 2mm of truth (guards against wrong-vertex events)
    if (std::abs(branch.recoVtxZ[0] - branch.truthVtxZ[0]) > .01f) continue;
    ...
    // tracks we will cluster
    std::vector<int> trks = getAssociatedTracks(&branch, MIN_TRACK_PT, MAX_TRACK_PT, 3.0);
    ...
    if (!all_clusters.empty()) {
      // Find cluster with highest TRKPTZ score (populated by updateScores inside clusterTracksInTime)
      auto best_cluster = chooseCluster(all_clusters, Score::TRKPTZ);

      pri_vtx_t = best_cluster.values[0];  // Time is first value
      pri_vtx_t_var = best_cluster.sigmas[0] * best_cluster.sigmas[0];
      clustering_succeeded = std::abs(pri_vtx_t - tru_vtx_t) < 60;
    } 

    // find connected tracks to the hard scatter vertex
    std::vector<int> connected_indices_z, connected_indices_trkptz_t, connected_indices_hgtd, connected_indices_truth_t;
	...
	continue;
      ...
      bool z_sig_cut = z_nsigma < max_nsigma_z;
    ...
    for (auto track_idx: connected_indices_z) {
      ...
      if (!apply_t_cut) {
	connected_indices_trkptz_t.push_back(track_idx);
	continue;
      }
      ...
      bool t_sig_cut = t_nsigma < max_nsigma_t;
	...
	continue;
      ...
      float t_nsigma = std::abs(dt / std::sqrt(hgtd_vtx_t_var + this_t_var));
      bool t_sig_cut = t_nsigma < max_nsigma_t;
	...
	PU_RpT_z_hist->Fill(jet_RpT_z);
      ...
      for (auto track_idx: connected_indices_trkptz_t) {
	double this_track_pt  = branch.trackPt [track_idx];
	double this_track_eta = branch.trackEta[track_idx];
	double this_track_phi = branch.trackPhi[track_idx];
  ...
  for (auto hist: {PU_RpT_t_hist, PU_RpT_hgtd_hist, PU_RpT_z_hist, HS_RpT_t_hist, HS_RpT_hgtd_hist, HS_RpT_z_hist, PU_RpT_truth_hist, HS_RpT_truth_hist}) {
  ...
  // z/z+time Comparison (PU)
  PU_RpT_z_hist->SetMaximum(5*std::max({old_pu_z_max,old_pu_t_max}));
  PU_RpT_z_hist->Draw("HIST");
  ...
  HS_RpT_z_hist->SetMaximum(5*std::max({old_hs_z_max,old_hs_t_max}));
