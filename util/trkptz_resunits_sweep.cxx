// trkptz_resunits_sweep.cxx
//
// 1D parameter sweep over the coefficient α in the modified TRKPTZ score:
//   Σpₜ · exp(−α · |Δz / σ_z|)
// where Δz/σ_z (delta_z_resunits) is the cluster z displacement from the
// reco vertex expressed in units of the cluster's own z uncertainty.
//
// This is analogous to trkptz_alpha_sweep but uses resolution-normalised
// units, making the weighting independent of the absolute z scale.
//
// Clustering is performed once per event at DIST_CUT_CONE (identical to
// the main analysis).  For each α the score is recomputed from the stored
// TRKPT score and a fresh Δz/σ_z (reads only trackZ0 and trackVarZ0),
// then the best cluster is selected and the standard sweep metrics are
// accumulated.
//
...
//
// Output PDF: ../parameter_sweep/trkptz_resunits_sweep.pdf
//
// ── Configuration ────────────────────────────────────────────────────────────
...
// ── Sweep configuration ───────────────────────────────────────────────────────
static constexpr Long64_t MAX_EVENTS     = -1;
static constexpr double   ALPHA_MIN      =  1.0;
static constexpr double   ALPHA_MAX      =  5.0;
static constexpr double   ALPHA_STEP     =  0.1;
static constexpr int      HS_TRACK_SPLIT =  5;
// ─────────────────────────────────────────────────────────────────────────────

// ── clusterDeltaZResUnits ─────────────────────────────────────────────────────
// Precision-weighted Δz (cluster z − reco vtx z) divided by the cluster's
// own z uncertainty σ_z = 1/√Σ(1/σ_i²).  Only reads trackZ0 and trackVarZ0.
// ─────────────────────────────────────────────────────────────────────────────
static double clusterDeltaZResUnits(const Cluster& c, const BranchPointerWrapper& b) {
  ...
  if (zden <= 0.f) return 0.0;
  float deltaZ = znum / zden - b.recoVtxZ[0];
  float zSigma = 1.f / std::sqrt(zden);
  return (double)(deltaZ / zSigma);
}

// ── chooseResUnitsAlpha ───────────────────────────────────────────────────────
// Select the cluster with the highest Σpₜ · exp(−α · |Δz/σ_z|).
// ─────────────────────────────────────────────────────────────────────────────
static Cluster chooseResUnitsAlpha(const std::vector<Cluster>& clusters,
  ...
  double bestScore = clusters[0].scores.at(Score::TRKPT.id)
                   * std::exp(-alpha * std::abs(clusterDeltaZResUnits(clusters[0], b)));
  for (const auto& c : clusters) {
    double s = c.scores.at(Score::TRKPT.id)
             * std::exp(-alpha * std::abs(clusterDeltaZResUnits(c, b)));
...
int main() {
  ...
  std::cout << "Score             : Σpₜ · exp(−α·|Δz/σ_z|)\n";
  ...
  for (int r = 0; r < N_REGIONS; ++r)
    for (int ia = 0; ia < nAlpha; ++ia)
      resHists[r][ia] = new TH1D(
        ...
        Form("TRKPTZ(res) #Deltat (%s, #alpha=%.2f);#Delta t [ps];Entries",
  ...
  while (reader.Next()) {
    ...
    for (int ia = 0; ia < nAlpha; ++ia) {
      Cluster best = chooseResUnitsAlpha(clusters, branch, alphas[ia]);

      resHists[region][ia]->Fill(best.values.at(0) - branch.truthVtxTime[0]);
  ...
  saveGraphPair(outFile, canvas, grTrackCount[0], grTrackCount[1],
    "Mean Track Count vs. #alpha (res-units score)",
    xtitle, "Mean N_{tracks} in selected cluster",
    xLo, xHi, 0.0, tcYMax * 1.2, HS_TRACK_SPLIT);

  // Residual shape overlay (sample every 5 steps)
  for (int r = 0; r < N_REGIONS; ++r) {
    ...
    for (int ia = 0; ia < nAlpha; ++ia) {
      ...
      h->SetTitle(Form("TRKPTZ(res) #Deltat shape (%s);#Delta t [ps];Normalised Entries",
                       regionLabel[r].c_str()));
  ...
  saveRocPlot(outFile, canvas,
    ...
    "Purity vs. Efficiency (#alpha res-units sweep)",
    "Overall Efficiency", "Mean Cluster Purity", HS_TRACK_SPLIT);

  saveRocPlot(outFile, canvas,
    sigLow, purLow, sigHigh, purHigh, alphaLabels,
    "Purity vs. Core #sigma (#alpha res-units sweep)",
    "Core #sigma [ps]", "Mean Cluster Purity", HS_TRACK_SPLIT);

  saveRocPlot(outFile, canvas,
    sigLow, effLow, sigHigh, effHigh, alphaLabels,
    "Efficiency vs. Core #sigma (#alpha res-units sweep)",
    "Core #sigma [ps]", "Overall Efficiency", HS_TRACK_SPLIT);
  ...
  for (int r = 0; r < N_REGIONS; ++r) {
    ...
    for (int ia = 0; ia < nAlpha; ++ia) {
      ...
      (void)dummy;
      std::cout << Form("%-8.2f  %-10.4f  %-10.2f  %-12.2f  %-10.4f  %-12.2f\n",
                        a, eff, sig, ncl, pur, ntk);
    }
  }
