...
namespace myutl {
  ...
  struct PlotObj {
    ...
    std::map<ScoreType,TH1D*> eff_pass;
    std::map<ScoreType,TH2D*> hist;
    std::map<ScoreType,TH2D*> purity;
    std::map<ScoreType,FitParams> params;
    std::map<ScoreType, std::vector<TH1D*>> slices_hists;
    std::map<ScoreType, std::vector<TF1*>> slices_fits;
    std::map<ScoreType,TEfficiency*> efficiency;
    TLegend* algolegend; // Legend for relevant algorithms used
  ...
  PlotObj(const char* title, const char* name, const char* fname,
    ...
    {
      algolegend = new TLegend(0.6, 0.7, 0.9, 0.9);
      algolegend->SetTextSize(0.03);
      for (ScoreType score: enum_vec) {
	hist[score] = new TH2D(Form("%s_%s",name,toString(score)),
	...
	purity[score] = new TH2D(Form("cluster_purity_%s_%s",name,toString(score)),
				 Form("%s Cluster Purity vs %s ;%s;Cluster Purity",toString(score), title,title),
				 (int)((x_max-x_min)/x_wid), x_min, x_max,
				 (int)((p_max-p_min)/p_wid), p_min, p_max);
	
	eff_pass[score] = new TH1D(Form("good_%s_%s",name,toString(score)),
    ...
    inline void plot_postprocessing() {
      TH2D *main2D = hist.at(ScoreType::HGTD);

      auto ytitle = main2D->GetYaxis()->GetTitle();

      for (ScoreType score: enum_vec) {
	TH2D* big_hist = hist.at(score);
	for(int j = 0; j < big_hist->GetNbinsX(); ++j) {
	  ...
	  TH1D* hSlice = (TH1D*)big_hist->ProjectionY(name, j+1, right)->Clone();

	  if (hSlice->Integral() == 0 || hSlice->GetEntries() < 6) {
	    delete hSlice; // Clean up the cloned object
	    continue;
	  }
	  ...
	  params.at(score).fill_each(j, dgaus_fit);

	  slices_hists.at(score).push_back(hSlice);
	  slices_fits.at(score).push_back(dgaus_fit);
    ...
    inline void plot_logic(TCanvas* canvas) {
      ...
      for (const auto& [score,entry] : this->hist) {
	entry->GetXaxis()->SetRangeUser(entry->GetXaxis()->GetXmin(), fold_max);
	entry->Draw("COLZ");
	canvas->Print(Form("%s",fname));
      }
	...
	TEfficiency* eff = new TEfficiency(*this->eff_pass.at(score), *this->eff_total);
	...
	eff->SetLineWidth(2);							 
	eff->SetMarkerColor(colors[score]);
	eff->SetLineColor(colors[score]);
	eff->SetLineWidth(2);
	if (first) {
	  eff->Draw("AP");
	...
	canvas->Print(Form("%s",fname));
      ...
      for (auto score: enum_vec) {
	std::vector<TF1*> fits = slices_fits.at(score);
	std::vector<TH1D*> slices = slices_hists.at(score);
	for (int i=0; i < fits.size(); i++) {
	  auto fit = fits[i];
	  auto hSlice = slices[i];
	  ...
	  canvas->Print(Form("%s",fname)); // slices
